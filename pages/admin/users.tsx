...
import type { UserForAdmin, UserNotificationForAdmin } from "../../graphql/adminUsers";
import {
  removeUser,
  setUserPaid,
  toggleSurvivor,
  ...
  const updateUserPaid = async (
    ...
    try {
      mutate(data => {
        ...
        const getUsersForAdmins = data.getUsersForAdmins.map(
          (user: { userID: number; userPaid: number }) => {
      ...
      logger.error({
        text: "Error updating user amount paid",
        amountPaid,
        error,
        userID,
      });
