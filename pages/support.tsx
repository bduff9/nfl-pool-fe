...
import type { Faq, Rule } from "../generated/graphql";
...
const Support: FC<SupportProps> = ({ faqs, rules, slackLink, supportEmail }) => {
  ...
  const logSupportSearch = debounce(1000, async (value: string): Promise<void> => {
    try {
      await writeSupportLog(LogAction.SupportSearch, (session?.user as User)?.id, value);
    } catch (error) {
