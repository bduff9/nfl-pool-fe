    ...
    async (result: DropResult): Promise<void> => {
      ...
      try {
        ...
        picksMutate(data => {
          ...
          const getMyPicksForWeek = data.getMyPicksForWeek.map(
            (pick: {
              game: {
                gameKickoff: string | number | Date;
                gameID: number | null;
                homeTeam: { teamID: number | null };
                visitorTeam: any;
              };
              pickPoints: number;
            }) => {
  ...
  const resetPicks = async (): Promise<void> => {
    try {
      ...
      picksMutate(data => {
        ...
        const getMyPicksForWeek = data.getMyPicksForWeek.map(
          (pick: { game: { gameKickoff: string | number | Date } }) => {
            if (new Date(pick.game.gameKickoff) > new Date()) {
              return {
                ...pick,
                pickPoints: null,
                  ...
                  picksData.getMyPicksForWeek.map(pick => (
                    <>
                      <PickGame
                        ...
                        isBackgrounded={
                          !!selectedGame && pick.game.gameID !== selectedGame
                        }
